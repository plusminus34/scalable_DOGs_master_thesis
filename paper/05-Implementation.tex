...
\begin{figure} [h]
	...
	\caption{A folding edge constraint defined on a blue crease point splitting the blue edge and degenerate cases where we do not enforce the constraint. From left to right: A regular folding edge constraint, degenerate osculating plane as $e_b,e_f$ are almost collinear, degenerate edges as one part of the edge split by the blue crease point is comparably very short, and lastly a degenerate angle between $e_b$ and the DOG edge crossing the blue point. An angle degeneracy often occurs before or after an edge degeneracy.}
...
\MiR{Mean curvature is quadratic under isometry (or even under same ratio, similar to Wardetzky's observance but true for the shape2018 paper as well), then iso energy (convex hessian). Then positional constraints by linear constraints (which are sometimes build as a curve constrained flow by slowly interpolating it). Then dihedral angle and fold bias are all written down as constraints and used as a quadratic objective with a Gauss-newton hessian. They also all start with zero deviation.}