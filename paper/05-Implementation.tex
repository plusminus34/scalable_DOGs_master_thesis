...
\end{equation}
Throughout the paper, unless stated otherwise, we use $w_\text{H} = 1$, $w_\text{pos}=5$, $w_\text{D} = 100$, $w_\text{iso}= \frac{20000}{\|E\|}$, $w_\text{cont} = \text{1e4}$, where $\|E\|$ is the number of edges in the net mesh (i.e., for a mesh with $1000$ vertices $w_\text{iso}=20$). Our meshes are always scaled to have an average edge length of $1$ and therefore using a different resolution for the same geometry keeps our bending objective the same, but scales the isometric objective by the number of edges.