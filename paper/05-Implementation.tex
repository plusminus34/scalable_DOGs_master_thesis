...
\end{split}
\end{align}
using the fixed parameters $h=1000,\delta = 1e-5$. We note that we do not use the common approximation for the Heaviside function $H(x) \approx \hat{H}(x) =  \frac{1+\tanh(hx)}{2}$ as $\hat H(0) = \frac{1}{2}$ instead of $0$. Since we use a homotopy based optimization it is important that our approximation for $H(x)$ will exactly vanish on a flat mesh.
