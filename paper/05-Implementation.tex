...
\end{enumerate}
We use the constants $\kappa_\epsilon=\text{1e-5}, \eps_r=5, \eps_\alpha=3^{\circ}$. We denote the folding constraints and the mountain/valley constraints (\equref{eq:mountain_valley}) by $\phi_{f_j}(\x) = 0, 1 \leq f_j \leq n_f$.
...
\newt{We only enforce the DOG angle constraints and the folding and mountain/valley constraints as hard constraints. The objective function includes a bending objective, soft positional constraints, soft dihedral constraints and a soft patches continuity constraint.}
...
\end{aligned}
\end{equation}
where $f$ is an objective function composed of a weighted sum of a bending objective, isometry objective, soft positional constraints and other terms as specified in \secref{sec:dog_obj}. \newt{We handle the combinatorial folding constraints by using a penalty based method (\secref{opt:folding}), and solve our problem with an iterative sequential quadratic programming (SQP) solver with a line search (\secref{sec:sqp_alg}).}
