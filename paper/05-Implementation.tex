...
\newt{We employ the tools developed in \secref{sec:folding} and \secref{sec:folding_angles_mountain_valley} to devise a simple folding and bending algorithm for deforming piecewise DOGs.} The algorithm aims to minimize an objective function while keeping the DOG constraints and ensuring the formation of folds along all crease curves \newt{and a given m/v assignment on a creases when such an assignment is given as input.} 
...
\end{enumerate}
We use the constants $\kappa_\epsilon=\text{1e-5}, \eps_r=5, \eps_\alpha=3^{\circ}$. We denote the set of all folding constraints and the mountain/valley constraints (\equref{eq:mountain_valley}) by $\phi_{f_j}(\x) = 0, 1 \leq f_j \leq n_f$.
...
\newt{We only enforce the DOG angle constraints and the folding and mountain/valley constraints as hard constraints. The objective function which we denote by $f(\x)$ is composed of a weighted sum of a bending objective, soft positional constraints, soft dihedral constraints and soft patches continuity constraints.}

Isometry is enforced as a soft constraint as advised by the degrees of freedom analysis in \cite{rabi18,rabi2018shape}, but we emphasize that all our results have an average relative edge stretch that is less than $0.003$, and a maximum stretch below \newt{$0.004$}, where our surfaces are normalized to have an average edge length of $1$. As opposed to \cite{rabi2018shape}, we also encode the linnear continuity constraints between patches as a soft constraint, as we have noticed a significant improvement in the quality and smoothness of crease patterns when these are enforced as a soft penalty with a large weight, and our results have an average continuity deviation of $0.0002$ and a maximum of $0.0035$. We note that the constrained shape space analysis in \cite{rabi2018shape} only concerns the DOG angle constraints, and complicated crease patterns give rise to a large set of additional linear constraints.
...
\end{aligned}
\end{equation}
\newt{where $f$ is specified in \secref{sec:dog_obj}. We handle the combinatorial constraints $\phi_{f_j}$ by using a penalty based method (\secref{opt:folding}), and solve our problem with an iterative sequential quadratic programming (SQP) solver with a line search (\secref{sec:sqp_alg}). The line search strategy ensures that the DOG angle constraints $\phi_{d_i}$ are satisfied numerically while the combinatorial constraints $\phi_{f_j}$ are satisfied exactly.}

\subsection{Objectives and \newt{convex hessian approximations}} \label{sec:dog_obj}
...
\subsection{Folding constraints} \label{opt:folding}
...
\begin{figure} [h]
	...
	\caption{Using the optional folding angle constraints (\secref{sec:folding_angle}) on a sparse set of crease points. These examples are deformed by constraining the folding angle of a set of points (in green), without specifying their folding orientation, and by setting a single positional constraint (in blue).}
...
\subsection{Equality constrained SQP} \label{sec:sqp_alg}
...
\end{equation}
%
where $J(\x)$ is the Jacobian of the equality constraints in \equref{eq:const_opt_penalty}, $\Delta^2_{xx}\mathcal{L}(x,\lambda) = H_{f_\omega}(x)+\sum\lambda_i^{k} \Delta^2_{xx} \phi_{d_i}(x)$ is the \newt{Hessian of the} Lagrangian of the problem and $H_{f_\omega}(\x)$ is the Hessian of $f_\omega(\x)$.
