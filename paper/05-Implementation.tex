...
\end{equation}
And note that under isometry deformations $t_1,t_2$ are linear in the vertices locations, $\alpha$ is fixed and the constraint is quadratic. \\
% squared half angle of two vectors is quadratic in their position to reach the following objective:
...
\end{equation}
% Write down stitching constraint
% Write down that we use them as soft cosntraint, and we use the gauss-newton hessian of the constraints as a convex hessian approximation
%Isometry is modeled in the objective as a soft constraint, as advised by the degrees of freedom analysis in \cite{rabi18,rabi2018shape}, but we emphasis that all of our results have an averaged relative edge stretch that is less than $0.003$ (\MiR{I can push it more with a stronger weight if needed}). 
%Likewise, we have noticed significant improvement while using soft stitching constraints \MiR{can we shortly discuss that?}.
%Our bending, positional constraints and dihedral angle objectives are all convex, while other parts of our objectives are not. of them are quadratic and convex besides the stretch objective and therefore we replacte its Hessian by a convex approximation of it.

\MiR{Add the convex approximation of the Hessian.}
% All of them are quadratic and convex besides the stretch objective and therefore we replacte its Hessian by a convex approximation of it.
