...
\end{equation}
Let $c(i)$ be a crease point, $e^1(i)$ and $e^2(i)$ its duplicated edges as in figure \figref{fig:fold_angle_and_tangent_angles}, and let $\theta_i$ be a desired dihedral angle along the fold at $c(i)$. We employ \lemmaref{lem:tangents_dihedral} to constrain the fold dihedral angle at $c(i)$ using a soft penalty, but write it down as a quadratic objective:
\begin{equation}
f_\text{bending}(\x) = 0.5\x^t(L^tM^{-1}L)\x
\end{equation}
...
Let $e$ be an edge on the mesh, $l_e$ its length and $l_e^0$ the length in a reference mesh. We define the following isometry constraints:
\begin{equation}
