...
\cite{Rabinovich:DogNets:2018}'s approach to modeling developable surfaces has advantages to other, previous methods in terms of flexibility.  had interactive deformations of developable surfaces before, but they relied on explicit rulings and splitting the mesh into regions that can only deform in certain ways. DOGs do not depend on such predefined structures, making it possible to explore the full shape space of possible deformations without having to change the mesh topology.\newline %TODO rewrite ;include more references like \cite{liu_et_al}
%TODO Domain decomposition and multigrid methods were possible ways to go. Multigrid requires refinement and coarsening, which are hard to with DOGs, so we went with domain decomposition. \cite{DDM_book} says we use a non-overlapping method with (later on) a coarse global solver, so okay %TODO this part man
% \cite{DDM_book} is a book about domain decomposition ...i didnt check it too much...
% initially: multigrid or domain decomposition. multigrid needs extra stuff, so domain decomposition seems easier to use. From domain decomposition it went to admm, admm also led to anderson acceleration.  then admm turned out to be crap and adding a coarse solver was good. but that's no longer previous work, is it
%the book says something about coarse
%include short problem description here? it's finite elements, optimization, with constraints
...
\cite{DDM_book} and \cite{unused_paper} seemed to be good starting points but then the focus shifted to the alternating direction method of multipliers (ADMM) as taken from \cite{Deng2017ParallelMA} and from there to Anderson acceleration \cite{Peng_2018} and \cite{Zhang_2019}.%  %TODO this part still sucks
%the bases for domain decomposition, then \cite{Deng2017ParallelMA} for ADMM and
%The main task lies in solving a highly constrained optimization problem. There is already a solver (using SQP) but it's so serial. Two main directions for improvement were there: Domain decomposition or multigrid methods. Multigrid needs coarsening and/or subdivision, which is a bit of a problem for DOGs. Furthermore, the mesh was already split into multiple regions naturally by the creases. Domain decomposition it is! \cite{DDM_book} includes a lot of stuff but the next most interesting thing was ADMM (TODOcite) and from there Anderson acceleration (TODOcite)...
