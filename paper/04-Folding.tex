...
Let $S(t)$ be a curved folding flow and let $p(t_k)$ be some crease point on a crease of $S(t)$ lying on two patches $P_1,P_2$ in a given time in the flow $t=t_k$. Assuming $p(t_k)$ is not a planar point on $P_1$ (or equivalently on $P_2$), then there exists an $\epsilon > 0$ such that one of the following holds:
\begin{enumerate}
...
\end{theorem}
\begin{proof}
Let $k(p(t))$ be the crease curvature at the the point, and let $k_g(p(t)) = k(p(t))cos\alpha(p(t))$ be the crease flattened geodesic curvature. If $\alpha \neq 0$ then the claim follows from the discontinuity of the two possible tangent planes along $p(t)$: a folding corresponds to a different choice of the tangent planes, whose angle is $2\alpha(p(t)$, and any small continuous transformation cannot move from a folded to a non folded configuration or vice-versa. Finally, the non planarity of $p(t_k)$ implies $\alpha \neq 0$, as the other option would be that the tangent of the crease curve is parallel to the ruling direction which by Lemma 12 and Corollary 16 in \cite{demaine_lens} implies that curve is kinked $p(t)$, thus the deformation is not piecewise $C^2$.
\end{proof}

Therefore it is impossible to fold a crease point that is non-planar. On a non-planar point that is not folded any small deformation keep it that way. Folding can only happen after flattening the point, and if a surface is already folded any small deformation keep it that way. Thus, it is crucial to enforce this for flat points, while no extra should be taken care for already folded creases. With this in mind, we note the following observation:

\begin{theorem}\label{Thm:supporting_plane}
A non-planar crease point is folded if and only if its osculating plane at that point is locally a supporting plane for the surface (see \figref{fig:plane_side}).
\end{theorem}
