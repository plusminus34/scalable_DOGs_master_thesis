...
A crease is 'active' on a curved fold if and only if the osculating plane of the crease curve is locally a supporting plane for the surface (see \figref{fig:plane_side}).
\end{theorem}
This follows directly from the fact that the tangent planes along a curved folded surface are reflections of one another through the curve's osculating plane.
