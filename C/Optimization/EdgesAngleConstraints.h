...
public:
	...
	virtual PointAngleConstraints* clone() const {return new PointPairConstraints(*this);}
	...
	virtual void updateJacobianIJV(const Eigen::VectorXd& x) {
		...
		for (int i = 0; i < pairs.size(); i++) {
			...
			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, v1_i, w1_x-w2_x);
			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, v1_i+vnum, w1_y-w2_y);
			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, v1_i+2*vnum, w1_z-w2_z);

			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, v2_i, -w1_x+w2_x);
			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, v2_i+vnum, -w1_y+w2_y);
			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, v2_i+2*vnum, -w1_z+w2_z);

			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, w1_i, v1_x-v2_x);
			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, w1_i+vnum, v1_y-v2_y);
			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, w1_i+2*vnum, v1_z-v2_z);

			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, w2_i, -v1_x+v2_x);
			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, w2_i+vnum, -v1_y+v2_y);
			IJV[ijv_cnt++] = Eigen::Triplet<double>(const_n, w2_i+2*vnum, -v1_z+v2_z);
