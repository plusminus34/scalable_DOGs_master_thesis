...
void PardisoSolver::set_system_matrix(const Eigen::SparseMatrix<double>& A) {
    Eigen::SparseMatrix<double,Eigen::RowMajor> M = A.cast<double>();
    set_system_matrix_row_major(M);
  };

  void PardisoSolver::set_system_matrix_row_major(const Eigen::SparseMatrix<double,Eigen::RowMajor>& A) {
    nnz = A.nonZeros();
    n = A.rows();

    ia.resize(A.outerSize()+1);
    ja.resize(nnz);
    a.resize(nnz);

    const int* out = A.derived().outerIndexPtr();
    for(int i=0;i<A.outerSize()+1;i++)
      ia[i] = out[i]+1;

    const int* in = A.derived().innerIndexPtr();
    for(int i=0;i<A.nonZeros();i++)
      ja[i] = in[i]+1;

    int i=0;
    for(int k=0; k<A.outerSize(); ++k)
      for(Eigen::SparseMatrix<double,Eigen::RowMajor>::InnerIterator it(A.derived(),k); it; ++it)
        a[i++] = it.value();
  };

  void PardisoSolver::update_system_matrix(const Eigen::SparseMatrix<double>& A) {
    Eigen::SparseMatrix<double,Eigen::RowMajor> M = A.cast<double>();
    update_system_matrix_row_major(M);
  }

  void PardisoSolver::update_system_matrix_row_major(const Eigen::SparseMatrix<double,Eigen::RowMajor>& A) {
    int i=0;
    for(int k=0; k<A.outerSize(); ++k)
      for(Eigen::SparseMatrix<double,Eigen::RowMajor>::InnerIterator it(A.derived(),k); it; ++it)
        a[i++] = it.value();
