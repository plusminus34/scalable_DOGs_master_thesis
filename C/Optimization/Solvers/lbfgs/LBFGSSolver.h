...
public:
    ...
    inline int minimize(Foo& f, Vector& x, Scalar& fx)
    {
        ...
        int end = 0;
        igl::Timer timer;
        double ls_t = 0; double f_t = 0; double g_t = 0; double dp_t = 0; double rst_t = 0; double tmp = 0;
        for( ; ; )
        {
            ...
            if(gnorm <= m_param.epsilon * std::max(xnorm, Scalar(1.0))) {
                cout << "Backtracking t = " << ls_t << endl;
                cout << "rest t = " << rst_t << endl;
                cout << "iter = " << k << endl;
                return k;
            }
