...
Eigen::VectorXd StitchingConstraints::Vals(const Eigen::VectorXd& x) const {
	...
	for (int i = 0; i < eS.edge_const_1.size(); i++) {
		...
		constVals(const_cnt) = 0;//0.5*(e1_c*x(e1.v1+0)+e2_c*x(e1.v2+0)-e1_c*x(e2.v1+0)-e2_c*x(e2.v2+0));
		constVals(const_cnt+1) = 0;//0.5*(e1_c*x(e1.v1+1*vnum)+e2_c*x(e1.v2+1*vnum)-e1_c*x(e2.v1+1*vnum)-e2_c*x(e2.v2+1*vnum));
		constVals(const_cnt+2) = 0;//0.5*(e1_c*x(e1.v1+2*vnum)+e2_c*x(e1.v2+2*vnum)-e1_c*x(e2.v1+2*vnum)-e2_c*x(e2.v2+2*vnum));
