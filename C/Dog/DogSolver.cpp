...
void DogSolver::single_iteration_fold(double& constraints_deviation, double& objective) {
  ...
  obj.compObj.update_weights({p.bending_weight,p.isometry_weight/dog.getQuadTopology().E.rows(), p.isometry_weight/dog.getQuadTopology().E.rows(), p.soft_pos_weight, p.soft_pos_weight, 0.1*p.soft_pos_weight, p.dihedral_weight, p.dihedral_weight, p.fold_bias_weight, p.mv_bias_weight, p.wallpaper_curve_weight/*,1000*p.wallpaper_curve_weight*/});
