...
class CurvedFoldingBiasSignObjective: public Objective {
public:
	CurvedFoldingBiasSignObjective(double alpha = 1, bool dbg = false) : alpha(alpha),dbg(dbg) {}
	void add_fold_bias(const CurvedFoldBias& foldBias);
	void reset_folds();
	
	virtual CurvedFoldingBiasSignObjective* clone() const {return new CurvedFoldingBiasSignObjective(*this);}
