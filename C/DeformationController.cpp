...
void DeformationController::init_curved_fold_from_given_mesh() {
	...
	for (int coord = 0; coord < 3; coord++) {
		for (int i = submesh_min_i; i <= submesh_max_i; i++) {
			curve_init_b(cnt) = coord*subm_size+i; curve_init_bc(cnt) = V(i,coord); 
			cnt++;
		}
	}
...
void DeformationController::optimize_curved_fold_initialization() {
	...
	// Objectives (bending, isometry, positional constraints for submesh 0)
	SimplifiedBendingObjective bending(quadTop, curved_init_x0); IsometryObjective isoObj(quadTop, curved_init_x0);
	PositionalConstraints posConst(curve_init_b,curve_init_bc);
	QuadraticConstraintsSumObjective posConstSoft(posConst, curved_init_x0);
...
void DeformationController::get_curve_fold_bias_obj() {
	bool dbg = true;
	CurvedFoldingBiasObjective tmpCurveFoldBiasObj(sign_opt_alpha, false,dbg);
	CurvedFoldingBiasObjective tmpCurveSignBiasSignObj(sign_opt_alpha,true,dbg);
...
/*
	for (int i = 0; i < 2; i++) {
	int fold_curve_idx = i; int e_idx; 
	e_idx=globalDog->getEdgeStitching().stitched_curves[i].size()/3;
