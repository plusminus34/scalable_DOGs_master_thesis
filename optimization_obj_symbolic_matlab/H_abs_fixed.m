...
IntegratedH = simplify((s2*cos_half_b1+s1*cos_half_b2).^2);
IntegratedKx = simplify((s1*cos_half_b1).^2); % not sure what's the best option here

H_new_check = simplify(cos_half_b1.^2);
ccode(H_new_check ,'file','mean_curve_not_integrated');
ccode(gradient(H_new_check ),'file','mean_curve_not_integrated_g');

ccode(IntegratedH ,'file','Integrated_H_abs_squared_E');
ccode(gradient(IntegratedH ),'file','Integrated_H_abs_squared_G');

ccode(IntegratedKx,'file','Kx_integrated_squared_E');
ccode(gradient(IntegratedKx),'file','Kx_integrated_squared_G');
%H = hessian(Kx_other);
