...
delta = sym('delta', 'real'); assume(delta>0);

const_check  = piecewise(delta <=1, delta, delta>0, 4);
ccode(const_check  ,'file','MV_fold_const2');
...
heaviside_right_side_approx = (tangent_tangent_plane_dot.^2)/((tangent_tangent_plane_dot.^2+delta));
mv_const = piecewise(tangent_tangent_plane_dot<=0, 0, tangent_tangent_plane_dot>0, heaviside_right_side_approx);
